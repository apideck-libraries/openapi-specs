...
- Added [Bill Credit Notes](apis/accounting/reference/bill-credit-notes) support for [Exact Online (NL)](connectors/exact-online-nl) — list, get, create, update and delete. Exact records an AP supplier credit note as a purchase entry whose amounts are credits rather than as a separate document type, so the list returns only credit notes and never the purchase invoices served by [Bills](apis/accounting/reference/bills); fetching a bill's id through this resource returns `404` rather than the same record relabelled. Creating one requires a purchase journal code on the connection setting `purchase_journal_code`. Because Exact derives the document type from the amounts themselves, `line_items[].total_amount` must be a positive amount of at least one cent — a zero, negative or sub-cent line would silently record a bill instead of a credit note and is rejected, as is a header `total_amount` that disagrees with the sum of the lines. Line amounts are always booked excluding tax and `tax_inclusive` is ignored, so tax from the line's `tax_rate.code` is added on top: a line of `100` with a 21% code produces a credit note whose `total_amount` is `121`, and this holds even for a tax code Exact presents as tax-inclusive. Updates are header-level only — `line_items` sent on a `PATCH` are ignored, so a line change means deleting and recreating. Filterable by `ids`, `number`, `updated_since`, `created_since` and `supplier_id`, sortable by `created_at`, `updated_at` and `id`. `status` reflects the entry's own state: an open entry reads as `authorised`, a processed one as `paid`. Exact does not track settlement on a purchase entry, so `balance`, `remaining_credit`, `date_paid` and `allocations` are never returned.
- `id` and `total_amount` are no longer marked required on the [Bill Credit Notes](apis/accounting/reference/bill-credit-notes) model. The document models it is built on — [Bills](apis/accounting/reference/bills), [Invoices](apis/accounting/reference/invoices) and [Purchase Orders](apis/accounting/reference/purchase-orders) — carry no required list, and it made the generated create request demand an `id` the caller cannot know and a `total_amount` the ledger is supposed to compute from the lines. Generated SDK models now type both as optional. No connector's behaviour changes: both fields are still returned on every read where the connector populates them. The AR twin [Credit Notes](apis/accounting/reference/credit-notes) still declares both required, so the two now differ; aligning them is tracked separately.
- Added [Bill Credit Notes](apis/accounting/reference/bill-credit-notes) support for [NetSuite](connectors/netsuite), backed by NetSuite Vendor Credits — with list, get, create, update and delete. Carries a `supplier` counterparty, `type: accounts_payable_credit`, line items, and `allocations[]` to a bill, expense report, journal entry, bill credit or vendor prepayment. Filterable by `updated_since`, `supplier_id` and `subsidiary_id`, and sortable by `created_at`, `updated_at` and `id`.
...
- Added new [e-Boekhouden](connectors/e-boekhouden) connector (beta) for the Dutch e-Boekhouden.nl bookkeeping platform — consumers connect with a personal API token from their own administration, with no partnership or app registration. Read and create support for [Bills](apis/accounting/reference/bills), [Invoices](apis/accounting/reference/invoices), [Journal Entries](apis/accounting/reference/journal-entries), [Payments](apis/accounting/reference/payments) and [Bill Payments](apis/accounting/reference/bill-payments); these have no update or delete, so corrections are booked as a reversing entry. List, get, create and update for [Ledger Accounts](apis/accounting/reference/ledger-accounts), [Customers](apis/accounting/reference/customers) and [Suppliers](apis/accounting/reference/suppliers); full create, read, update and delete for [Tracking Categories](apis/accounting/reference/tracking-categories); and read support for [General Ledger Transactions](apis/accounting/reference/general-ledger-transactions), [Aged Creditors](apis/accounting/reference/aged-creditors) and [Aged Debtors](apis/accounting/reference/aged-debtors). Worth knowing before you build against it: e-Boekhouden stores no customer/supplier flag on a contact, so `customers` and `suppliers` return the same list; payments and bill payments reference their invoice or bill by its human-readable number in `allocations[].id`, and the allocations must add up to `total_amount`; the bills list is a summary view, with `line_items`, `total_tax`, `notes`, `supplier` and `due_date` available from the get-by-id call; tax rates are not exposed as a resource, since e-Boekhouden's VAT codes are a fixed set defined by Dutch tax law and are passed as `tax_rate.code` on each line. No webhooks — e-Boekhouden's API exposes no last-modified timestamp on any resource, so polling-based emulation isn't possible either.
- Added the [Bill Credit Notes](apis/accounting/reference/bill-credit-notes) resource — supplier-issued (accounts payable) credit notes, the AP-side counterpart to [Credit Notes](apis/accounting/reference/credit-notes). It carries a `supplier` counterparty, `line_items`, and `type: accounts_payable_credit`, and is first implemented on [Microsoft Dynamics 365 Business Central](connectors/microsoft-dynamics-365-business-central) (backed by Business Central Purchase Credit Memos) with read and write support. Other connectors report the resource as unsupported until mapped. The existing Credit Notes resource is unchanged and remains accounts-receivable (customer-side).
...
- Added new [Sage Intacct (REST)](connectors/sage-intacct-rest) connector, covering [Customers](apis/accounting/reference/customers), [Suppliers](apis/accounting/reference/suppliers), [Invoices](apis/accounting/reference/invoices), [Bills](apis/accounting/reference/bills), [Credit Notes](apis/accounting/reference/credit-notes), [Payments](apis/accounting/reference/payments), [Bill Payments](apis/accounting/reference/bill-payments), [Refunds](apis/accounting/reference/refunds), [Journal Entries](apis/accounting/reference/journal-entries), [Ledger Accounts](apis/accounting/reference/ledger-accounts), [Tax Rates](apis/accounting/reference/tax-rates), [Quotes](apis/accounting/reference/quotes), [Expenses](apis/accounting/reference/expenses), [Expense Reports](apis/accounting/reference/expense-reports), [Expense Categories](apis/accounting/reference/expense-categories), [Invoice Items](apis/accounting/reference/invoice-items), [Projects](apis/accounting/reference/projects), [Departments](apis/accounting/reference/departments), [Locations](apis/accounting/reference/locations), [Tracking Categories](apis/accounting/reference/tracking-categories), [Subsidiaries](apis/accounting/reference/subsidiaries), [Companies](apis/accounting/reference/companies), [Company Info](apis/accounting/reference/company-info), [Employees](apis/accounting/reference/employees), [Bank Accounts](apis/accounting/reference/bank-accounts), [Bank Feed Statements](apis/accounting/reference/bank-feed-statements) and [Attachments](apis/accounting/reference/attachments), plus virtual (polling-based) webhook events for created, updated and deleted across 13 of those resources.
- Added [Quotes](apis/accounting/reference/quotes) read and write support for [Sage Intacct (REST)](connectors/sage-intacct-rest) — the REST API only accepts order-entry document writes on a type-qualified path, and the document type is defined per company, so quotes are resolved against the company's own Order Entry transaction definitions on each request. Requires the Order Entry module permission.
- Financial reports are not available on [Sage Intacct (REST)](connectors/sage-intacct-rest). Sage's REST v1 reporting endpoints return downloadable files asynchronously rather than queryable data, so balance sheet, profit and loss, aged debtors and aged creditors are served only by the XML-based [Sage Intacct](connectors/sage-intacct) connector.
- Known limitations on [Sage Intacct (REST)](connectors/sage-intacct-rest): at most one attachment is visible per record, `sub_total` equals `total` on invoices, bills and credit notes because Sage exposes no pre-tax subtotal, credit note amounts are signed as Sage stores them, the `x-apideck-company-id` header has no effect because Sage scopes entities per line rather than per request, and `status` accepts only `active` or `inactive` on writes.
...
- Added native webhook events support for [Orders](apis/ecommerce/reference/orders), [Products](apis/ecommerce/reference/products), and [Customers](apis/ecommerce/reference/customers) on the [BigCommerce](connectors/bigcommerce) connector, covering create, update, and delete events. BigCommerce has no order-deletion event, so `ecommerce.order.deleted` is emitted when an order is archived (`store/order/archived`).

## v10.28.0 - (2026-07-24)

### Issue Tracking API

- Added new [Jira (API Token)](connectors/jira-api-token) connector for connecting Jira Cloud through a service account (Atlassian email + API token, HTTP Basic auth) instead of OAuth 2.0, with support for [Collections](apis/issue-tracking/reference/collections), [Collection Tickets](apis/issue-tracking/reference/collection-tickets), [Collection Ticket Comments](apis/issue-tracking/reference/collection-ticket-comments), [Collection Users](apis/issue-tracking/reference/collection-users), and [Collection Tags](apis/issue-tracking/reference/collection-tags) resources, plus native webhook events for tickets.

## v10.27.0 - (2026-07-22)

### Accounting API

- Added `billed_since` and `due_since` filters to the [Bills](apis/accounting/reference/bills) resource for narrowing by document date and due date (on or after, `YYYY-MM-DD`). `billed_since` is supported on the [Workday](connectors/workday), [Fortnox](connectors/fortnox), [DualEntry](connectors/dualentry), [Campfire](connectors/campfire), [Procountor](connectors/procountor-fi), and [Yuki](connectors/yuki) connectors; `due_since` on [Workday](connectors/workday), [DualEntry](connectors/dualentry), and [Procountor](connectors/procountor-fi).
...
- Added optional `subsidiary` field ([LinkedSubsidiary](apis/accounting/reference/subsidiaries)) to [Invoices](apis/accounting/reference/invoices), [Bills](apis/accounting/reference/bills), [Credit Notes](apis/accounting/reference/credit-notes), [Journal Entries](apis/accounting/reference/journal-entries), [Payments](apis/accounting/reference/payments), and [Bill Payments](apis/accounting/reference/bill-payments) so transaction reads carry legal-entity attribution (`id`, `display_id`, `name`) alongside the existing `company_id` scalar. Populated on [Sage Intacct](connectors/sage-intacct) from `MEGAENTITY*`.
- Fixed [Sage Intacct](connectors/sage-intacct) [Subsidiaries](apis/accounting/reference/subsidiaries) to return only legal entities (Intacct `LOCATIONENTITY`) instead of all locations, matching the behavior of the [Companies](apis/accounting/reference/companies) endpoint.
- Fixed [Sage Intacct](connectors/sage-intacct) error responses to surface Intacct's native error code and message via `downstream_errors[]` (previously returned `null` with a raw XML dump). Closed-period and validation errors now expose `code: "BL01001973"` with the actionable description text.
- Fixed [Sage Intacct](connectors/sage-intacct) [Credit Notes](apis/accounting/reference/credit-notes) to actually create credit memos. Previous behavior sent the unified positive `total_amount` as-is to `create_aradjustment`, which Intacct interpreted as a debit memo. The connector now negates the line amount on write and normalizes back to positive on read so the consumer's view is unchanged.
...
- Documented per-tenant, per-country address-field behavior for [SAP SuccessFactors](connectors/sap-successfactors): `line1`–`line5` now pass through verbatim from SAP's positional `address1`–`address5`, `street_number` is not populated, and consumers must compose addresses client-side using `addresses[].country`.
...
- Added new [Spiris / Visma eAccounting](connectors/spiris) connector with support for [Customers](apis/accounting/reference/customers), [Suppliers](apis/accounting/reference/suppliers), [Invoices](apis/accounting/reference/invoices), [Bills](apis/accounting/reference/bills), [Credit Notes](apis/accounting/reference/credit-notes), [Payments](apis/accounting/reference/payments), [Bill Payments](apis/accounting/reference/bill-payments), [Journal Entries](apis/accounting/reference/journal-entries), [Ledger Accounts](apis/accounting/reference/ledger-accounts), [Tax Rates](apis/accounting/reference/tax-rates), [Invoice Items](apis/accounting/reference/invoice-items), [Bank Accounts](apis/accounting/reference/bank-accounts), [Tracking Categories](apis/accounting/reference/tracking-categories), [Company Info](apis/accounting/reference/company-info), and [Attachments](apis/accounting/reference/attachments) resources
